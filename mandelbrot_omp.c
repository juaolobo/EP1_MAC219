...
void compute_mandelbrot(){
    ...
    int omp_threads = atoi(OMP_THREADS);

    int work_amount = i_x_max / omp_threads;
    
    #pragma omp parallel \
        shared (i_y_max, i_x_max, c_x_min, c_y_min, pixel_height, pixel_width, iteration_max, escape_radius_squared, work_amount, omp_threads) \
        private (i_y, i_x, c_y, c_x, z_x, z_y, z_x_squared, z_y_squared, iteration) 
        {
            int threadID = omp_get_thread_num();
            int start = work_amount * threadID;
            int end = work_amount * (threadID + 1);

            if (threadID == omp_threads - 1)
                end = i_x_max;
